...
## MutexGuard or SemaphoreGuard?

These two libraries are deliberately similar — both are RAII guards over a
FreeRTOS `SemaphoreHandle_t` that take the lock in the constructor, release it
in the destructor, and forbid copy and move. Both ship a `Recursive*` variant
for recursive mutexes. They differ in **what happens when the lock is
contended**, and that difference is the whole basis for choosing:

| | **MutexGuard** | **SemaphoreGuard** |
|---|---|---|
| Default wait | **100 ms**, bounded | **`portMAX_DELAY`** — waits forever |
| Timeout is | opt-out (pass your own) | opt-in (one-arg ctor blocks forever) |
| ISR context | **Detected** via `xPortInIsrContext()`; refuses to block | Not detected |
| Early release | `unlock()` | none — scope only |
| `if (guard)` | `explicit operator bool` | use `hasLock()` |
| Handle access | none | `getHandle()` |
| Debug tooling | log level only | `SEMAPHORE_GUARD_DEBUG` records file/line + hold time, plus `SEMAPHORE_GUARD()` macros |

**Use `MutexGuard`** for anything on a safety or control path. The bounded
default means a lock-ordering mistake surfaces as a failed `hasLock()` you can
handle, not a task parked forever. The ISR check makes it safe to place in code
that might be reached from an interrupt.

**Use `SemaphoreGuard`** when the lock genuinely must be acquired before
proceeding and blocking is acceptable — initialisation, teardown, or background
work off the control path — or when you want its file/line contention
diagnostics to hunt down a slow lock.

> **Always check `hasLock()`.** With either guard, a timeout or a null handle
> leaves the guard constructed but *not* holding the lock. Proceeding into the
> critical section anyway is the most common way to misuse both libraries.

```cpp
MutexGuard lock(myMutex, pdMS_TO_TICKS(100));
if (!lock.hasLock()) {
    LOG_ERROR(TAG, "mutex timeout");
    return;                  // do NOT touch the shared data
}
// critical section
```
...
description of the change and any test results.